		...
		$.ajax({
			...
			success:function(data) {
				...
				for (var i=0; i<data.length; i++) {
					var thRow = '<th colspan="2">'+data[i].date+'</th>'
					$('#table-th').append(thRow);
				}
				
				for (var i=0; i<data.length; i++) {
					var tdRow = '<td>'+'현금결제 : '+data[i].credit+'건'+'</td>'
						tdRow += '<td>'+'카드결제 : '+data[i].card+'건'+'</td>'
						$('#table-td').append(tdRow);
				}
			}
		})
	})
	...
	$('#payment-status').on('click', function() {
		$.ajax({
			...
			success:function(data) {
				...
				for (var i=0; i<data.length; i++) {
					var thRow = '<th>'+data[i].date+'</th>'
					$('#table-th').append(thRow);
				}
					
				for (var i=0; i<data.length; i++) {
					var tdRow = '<td>'+data[i].payStatus+'건'+'</td>'
					$('#table-td').append(tdRow);
				}
			}
		})
	...
	$('#total-sales').on('click', function() {
		$.ajax({
			...
			success:function(data) {
				...
				for (var i=0; i<data.length; i++) {
					var thRow = '<th>'+data[i].lineDate+'</th>';
					$('#table-th').append(thRow);
				}
				
				for (var i=0; i<data.length; i++) {
					var tdRow = '<td>'+numberFormat(data[i].totalSales)+'원'+'</td>'
					$('#table-td').append(tdRow);
				}
			}
		})
